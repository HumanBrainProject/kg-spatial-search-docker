...
def print_point(oid, referenceSpace, point, end):
    """ Generate a single data point with the properties provided. """

    (x0, x1, x2) = point
    point_str = '{{"type":"Feature","geometry":{{"type":"Point","referenceSpace":"{}",'
    point_str += '"coordinates":["{},{},{}"]}},'
    point_str += '"properties":{{"id":"{}"}}}}'
    point_str += end

    print(point_str.format(referenceSpace, x0, x1, x2, oid))

def print_oid(referenceSpace, oid, num, end):
    ...
    while k < num:
        ...
        print_point(oid, referenceSpace, coordinates, end)
        k+=1

def print_space(referenceSpace, oids_per_space, points_per_oid):
    """ Generate a space ID with `oids_per_space` different OIDs, and 
        `points_per_oid` spatial records per OID.
    """
    i=0
    while i < oids_per_space - 1:
        oid=random.random()
        print_oid("space{}".format(referenceSpace), "oid{}".format(oid),
                  points_per_oid, ",")
        i+=1

    oid=random.random()
    print_oid("space{}".format(referenceSpace), "oid{}".format(oid), points_per_oid-1, ",")
    print_oid("space{}".format(referenceSpace), "oid{}".format(oid), 1, "")

def generate_file(points_per_oid, oids_per_space):
    print("[")
    referenceSpace=random.random()
    print_space(referenceSpace, oids_per_space, points_per_oid)
    print("]")
